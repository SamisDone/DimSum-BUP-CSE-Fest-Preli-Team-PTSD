...
## IMPORTANT:
Deployment, repository, submission, performance, scoring, tie-break, and general participation rules
...
## LLM REQUIREMENT:
The language model must be part of the operator-note interpretation path. Using an LLM only
...
## CORE IDEA:
Human notes are not directly trusted as math. They are first converted to a fixed structured format,
...
"Keep at least 120 kWh in reserve from 6 PM until 9
## PM."
minimum_battery_reserve; hours [18,19,20]; 120
kWh
...
Deterministic effect used by the optimizer
...
battery parameters unless a supported directive explicitly allows
it.
Final replay
...
feasible ground-truth interpretation and will not
require contradictory hard directives.

## SAFE FAILURE:
If the LLM returns malformed or unsupported structured output, the service must handle it in a
...
The starting battery may shift energy between hours, but it cannot be consumed as a
...
## NO BYTE-FOR-BYTE MATCHING:
Equivalent valid optimal schedules may differ. The judge evaluates structured
...
## FINAL REMINDER:
Understand the operator notes first. Validate the extracted directives. Apply them to the
